            ...
            console.print(
                Columns(
                    [
                        ...
                        templateContratStr(evenement['contrat']),
                        templateClientStr(evenement['client']),
                        templateUserStr(evenement['support_contact'], 1),
                        templateUserStr(evenement['contrat']['commercial_contact'])
                    ],
                    expand=True
                )
            )
...
def show():
    console = Console()
    conf = initConfig()
    response = myRequests("get", conf['url']+'api/evenements/all/', headers=conf["headers"])
    if not response:
        return False
    if statusOK(response):
        reponseJson = response.json()
        if len(reponseJson) == 0:
            console.rule("[red]0 evenement trouvé[red]", style="bold red", characters="=")
        else:
...
def showFilter():
    console = Console()
    conf = initConfig()
    response = myRequests("get", conf['url']+'api/evenements/', headers=conf["headers"])
    if not response:
        return False
    if statusOK(response):
        reponseJson = response.json()
...
def create(
    ...
    while 'date_start' not in data:
        ...
        try:
            data['date_start'] = datetime.strptime(
                date['date_start']+date['heure_start'], "%d/%m/%Y%H:%M"
            ).replace(tzinfo=user_timezone)
        except (ValueError, TypeError, KeyError, AttributeError):
            print(
                f"[bold red] Erreur dans les données rentrer pour la date de strat"
                f"{date['date_start']+date['heure_start']}. Exemple valide date : '21/8/2028' et heure : '15:30',"
                f"veuillez recommencé[/bold red]"
            )
    while 'date_end' not in data:
        date = {}
        ...
        try:
            data['date_end'] = datetime.strptime(
                date['date_end']+date['heure_end'], "%d/%m/%Y%H:%M"
            ).replace(tzinfo=user_timezone)
        except (ValueError, TypeError, KeyError, AttributeError):
            print(
                f"[bold red] Erreur dans les données rentrer pour la date de strat"
                f"{date['date_end']+date['heure_end']}. Exemple valide date : '21/8/2028' et heure : '15:30',"
                f" veuillez recommencé[/bold red]"
            )
    ...
    conf = initConfig()
    response = myRequests("post", conf['url']+'api/evenements/', headers=conf["headers"], data=data)
    if not response:
        return False
    if statusOK(response):
        reponseJson = response.json()
        if response.status_code == 201:
            console = Console()
            console.rule("[green]Evenement créé avec succes ![green]", style="bold green", characters="=")
...
def change(
    ...
    if qui == 'SU':
        ...
        print("[bold red]Vous n'avez pas ecrit une reponse que je connais ('GE' ou 'SU')[/bold red]")
        return None
    if data == {}:
        print("[bold red]Vous n'avez rien changer a l'evenement[/bold red]")
        return None
    conf = initConfig()
    response = myRequests("put", conf['url']+'api/evenements/'+dataUrl['id']+'/', headers=conf["headers"], data=data)
    if not response:
        return False
    if statusOK(response):
        reponseJson = response.json()
        if response.status_code == 200:
            console = Console()
            console.rule("[green]Evenement modifié avec succes ![green]", style="bold green", characters="=")
            templateEvenement(console, reponseJson)
            console.rule(style="bold green", characters="=")
        else:
            print(f"[bold red]{reponseJson['error']}[/bold red]")
